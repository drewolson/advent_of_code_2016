defmodule Program do
  def main do
    ...
    [{start_node, 0}]
    |> solve(graph, MapSet.new, count)
    |> IO.puts
  end

  defp solve([{{pos, goals} = state, steps}|rest], graph, visited, goal_count) do
    cond do
      Enum.count(goals) == goal_count ->
        steps
      Enum.member?(visited, state) ->
        solve(rest, graph, visited, goal_count)
      true ->
        new_states =
          pos
        ...
        solve(rest ++ new_states, graph, MapSet.put(visited, state), goal_count)
    end
  end
